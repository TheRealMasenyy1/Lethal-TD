...
local FlashlightPilot = {}
FlashlightPilot.__index = setmetatable(FlashlightPilot,UnitManager)


function FlashlightPilot.Setup(Unit)
	local self = UnitManager.new(Unit)
	setmetatable(self,FlashlightPilot)
	
	self.Animations = {
		Attack = self:LoadAnimation(EntityInfo["FlashlightPilot"].Attack);
		Idle = self:LoadAnimation(EntityInfo["FlashlightPilot"].Idle);
	}
	
	self.AttackSound = self:LoadSound(EntityInfo["FlashlightPilot"].Sound)
	
	--- Empty Constructor
	return self
end

function FlashlightPilot:Attack()
