...
local Flea = {}
Flea.__index = setmetatable(Flea,UnitManager)


function Flea.Setup(Unit)
	local self = UnitManager.new(Unit)
	setmetatable(self,Flea)
	
	self.Animations = {
		Attack = self:LoadAnimation(EntityInfo["Flea"].Attack);
		Idle = self:LoadAnimation(EntityInfo["Flea"].Idle);
	}
	
	self.AttackSound = self:LoadSound(EntityInfo["Flea"].Sound)
	
	--- Empty Constructor
	return self
end

function Flea:Attack()
