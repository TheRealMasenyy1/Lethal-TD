...
function ShotgunPilot.Setup(Unit)
	...
	self.Animations = {
		Attack = self:LoadAnimation(EntityInfo["ToySoldier"].Attack);
		Idle = self:LoadAnimation(EntityInfo["ToySoldier"].Idle);
	}
	
	self.AttackSound = self:LoadSound(EntityInfo["ToySoldier"].Sound)
