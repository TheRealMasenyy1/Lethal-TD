...
--ReplicatedStorage.SharedPackage.Animations
local UnitManager = require(ReplicatedStorage.Resources.UnitManager)

local Baboon = {}
Baboon.__index = setmetatable(Baboon,UnitManager)


function Baboon.Setup(Unit)
	local self = UnitManager.new(Unit)
	setmetatable(self,Baboon)
	
	self.Animations = {
		Attack = self:LoadAnimation(EntityInfo["Baboon"].Attack);
		Idle = self:LoadAnimation(EntityInfo["Baboon"].Idle);
	}
	
	self.AttackSound = self:LoadSound(EntityInfo["Baboon"].Sound)
	
	--- Empty Constructor
	return self
end

function Baboon:Attack()
